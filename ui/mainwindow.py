...
class MainWindow(FramelessWindow):
    ...
    def __init__(self, app: QApplication, config: ProgramConfig, open_dir='', *args, **kwargs) -> None:
        ...
        if open_dir != '' and osp.exists(open_dir):
            ...
            if len(pcfg.recent_proj_list) > 0:
                proj_dir = pcfg.recent_proj_list[0]
    ...
    def resetStyleSheet(self, reverse_icon: bool = False):
        theme = 'modern-dark' if pcfg.darkmode else 'eva-light'
    ...
    def setupUi(self):
        ...
        self.modernSidebar = ModernSidebar(self)
        self.modernSidebar.page_list_toggled.connect(self.pageLabelStateChanged)
        self.modernSidebar.search_toggled.connect(self.on_set_gsearch_widget)
        self.modernSidebar.action_open.connect(self.on_open_proj)
        self.modernSidebar.action_detect.connect(self.on_run_detect)
        self.modernSidebar.action_ocr.connect(self.on_run_ocr)
        self.modernSidebar.action_inpaint.connect(self.on_run_inpaint_mask)
        self.modernSidebar.action_batch.connect(self.show_batch_dialog)
        self.modernSidebar.action_export.connect(self.on_export_proj)
        self.modernSidebar.action_settings.connect(self.show_settings)

        self.batchDialog = BatchDialog(self)
        self.floatingToolbar = FloatingToolbar(self)
        self.floatingToolbar.hide()  # Show when canvas is active
        self.floatingToolbar.tool_changed.connect(self.on_tool_changed)
        self.floatingToolbar.action_inpaint.connect(self.on_run_inpaint_mask)
        self.floatingToolbar.action_clear.connect(self.on_clear_masks)
        ...
        mainHLayout = QHBoxLayout()
        mainHLayout.addWidget(self.modernSidebar)
    ...
    def setupConfig(self):
        ...
        self.modernSidebar.imgTransChecker.setChecked(True)
    ...
    def setupImgTransUI(self):
        ...
        self.floatingToolbar.show()
        # Ensure page list is visible if project loaded
    ...
    def dropOpenDir(self, directory: str):
        if isinstance(directory, str) and osp.exists(directory):
            self.modernSidebar.updateRecentProjList(directory)
            self.openDir(directory)

    def on_open_proj(self):
        """Open project - open folder dialog for selecting a project directory."""
        d = None
        recent_list = pcfg.recent_proj_list
        if len(recent_list) > 0:
            for projp in recent_list:
    ...
    def openJsonProj(self, json_path: str):
        try:
            ...
            self.st_manager.clearSceneTextitems()
            self.modernSidebar.updateRecentProjList(self.imgtrans_proj.proj_path)
            self.updatePageList()
    ...
    def pageLabelStateChanged(self):
        setup = self.modernSidebar.showPageListLabel.isChecked()
        if setup:
            if self.leftStackWidget.isHidden():
                self.leftStackWidget.show()
            if self.modernSidebar.globalSearchChecker.isChecked():
                self.modernSidebar.globalSearchChecker.setChecked(False)
    ...
    def resizeEvent(self, event):
        if hasattr(self, 'floatingToolbar'):
            # Position toolbar at top center, 20px down
            target_x = (self.width() - self.floatingToolbar.width()) // 2
            self.floatingToolbar.move(target_x, 60) # 60px from top (below titlebar)
    ...
    def setupShortcuts(self):
        ...
        self.titleBar.run_trigger.connect(self.modernSidebar.btn_ocr.click)
        ...
        shortcutSpace.activated.connect(self.shortcutDrawboard)
    ...
    def on_global_search(self):
        if self.canvas.gv.isVisible():
            if not self.modernSidebar.globalSearchChecker.isChecked():
                self.modernSidebar.globalSearchChecker.click()
            fo = self.app.focusObject()
            sel_text = ''
            blkitem = self.canvas.editing_textblkitem
            if fo == self.canvas.gv and blkitem is not None:
    ...
    def on_run_detect(self):
        """Run text detection pipeline with loading overlay."""
        self.setupImgTransUI()
        if self.imgtrans_proj.is_empty:
            QMessageBox.information(self, "Detect", "Please open a project first.")
            return
        self.loadingOverlay.showLoading("Detecting text...", "Running text detection on current page")
        # Run the full imgtrans pipeline (detect only)
        self.module_manager.runImgtransPipeline()

    def on_run_ocr(self):
        """Run OCR on detected text blocks with loading overlay."""
        self.setupImgTransUI()
        if self.imgtrans_proj.is_empty:
            QMessageBox.information(self, "OCR", "Please open a project first.")
            return
        self.loadingOverlay.showLoading("Running OCR...", "Recognizing text in detected regions")
        self.module_manager.runImgtransPipeline()
    ...
    def on_tool_changed(self, tool_mode):
        if tool_mode == 'select':
             self.canvas.setPaintMode(False)
             self.setTextEditMode() # Switch to text edit
        elif tool_mode == 'rect':
             self.setPaintMode()
             self.drawingPanel.setTool('rect')
        elif tool_mode == 'brush':
             self.setPaintMode()
             self.drawingPanel.setTool('pen')
        elif tool_mode == 'eraser':
             self.setPaintMode()
             self.drawingPanel.setTool('eraser')

    def on_run_inpaint_mask(self):
        self.setupImgTransUI()
        self.drawingPanel.rectPanel.inpaint_btn.click()
    ...
    def save_proj(self):
        if self.modernSidebar.imgTransChecker.isChecked()\
    ...
    def translatorStatusBtnPressed(self):
        self.modernSidebar.configChecker.setChecked(True)
    ...
    def on_imgtrans_pipeline_finished(self):
        self.loadingOverlay.hideLoading()
    ...
    def on_export_proj(self):
        """Export project - calls on_export_doc for now."""
        if self.loadingOverlay:
             self.loadingOverlay.showLoading("Exporting...", "Saving project files")
        self.on_export_doc()
    ...
    def on_set_gsearch_widget(self):
        setup = self.modernSidebar.globalSearchChecker.isChecked()
        if setup:
            if self.leftStackWidget.isHidden():
                self.leftStackWidget.show()
            self.modernSidebar.showPageListLabel.setChecked(False)
    ...
    def resizeEvent(self, event):
        if hasattr(self, 'loadingOverlay') and self.loadingOverlay.isVisible():
